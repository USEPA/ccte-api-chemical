...
@NamedNativeQuery(
        ...
        query = """
                 select cd.dtxsid,  cd.dtxcid, cd.generic_substance_id, cd.casrn, cd.preferred_name, cd.compound_id, cd.stereo, cd.isotope, \
                 ...
                 from ch.v_chemical_details  cd join (\s
                 select  dtxsid, dtxcid, search_name, search_value, modified_value,rank\s
                 from ch.v_chemical_search where modified_value = :searchWord ) sc on cd.dtxsid=sc.dtxsid or cd.dtxcid=sc.dtxcid \
                 order by rank, search_value asc\s\
                """
)
@NamedNativeQuery(
