...
@ActiveProfiles("test")
@WebMvcTest(
        controllers = {ChemicalPropertyResource.class},
        properties = "application.batch-size=2"
)
...
public class ChemicalPropertyResourceTest {
    ...
    void testGetExpermientalPropertiesByBatchDtxsid() throws Exception {
        final List<ChemicalPropertyExperimental> properties = Collections.singletonList(propertyExperimental);
        List<String> jsonArray = List.of("DTXSID7020182");
    ...
    @Test
    void testFatchBatchSearchRejectsMissingBody() throws Exception {
    mockMvc.perform(post("/chemical/fate/search/by-dtxsid/")
            .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.detail").
                value(Messages.REQUEST_BODY_MUST_NOT_BE_EMPTY_MSG));
    }

    @Test
    void testFateBatchSearchrejectsBatchLargerThanConfiguredSize() throws Exception {
        List<String> dtxsids = IntStream.rangeClosed(0, maxBatchSize)
            .mapToObj(index -> "DTXSID" + index)
            .toList();

    mockMvc.perform(post("/chemical/fate/search/by-dtxsid/")
            .contentType(MediaType.APPLICATION_JSON)
            .content(new ObjectMapper().writeValueAsString(dtxsids)))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.violations",
                hasItem(Messages.MAX_BATCH_SIZE_MSG.formatted(maxBatchSize))));
    }        
    @Test
    ...
    @Test
    void testFateBatchSearchRejectsBadlyFormattedDtxsidList() throws Exception {
        String jsonBody = "[\"DTXQRS7020182\", \"DTXSID9020112\"]";
        ...
        mockMvc.perform(post("/chemical/fate/search/by-dtxsid/")
                ...
                .andDo(MockMvcResultHandlers.print())
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail", containsString("Validation failure")))
                .andExpect(jsonPath("$.violations", hasItem(Messages.BAD_DTXSID_FORMAT_MSG)));

        verify(experimentalRepository, never()).findFateByDtxsidInOrderByDtxsidAsc(anyList());
    }
    
    @Test
    void testFateBatchSearchRejectsEmptyDtxsidList() throws Exception {
        String jsonBody = "[]";
        ...
        mockMvc.perform(post("/chemical/fate/search/by-dtxsid/")
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail", containsString("Validation failure")))
                .andExpect(jsonPath("$.violations", hasItem(Messages.EMPTY_BATCH_MSG)));
