...
public class ChemicalListResourceTest {
    ...
    private ChemicalListAll listAll;
    ...
    public void setup() {
    	...
    	chemicalList = ChemicalList.builder()
    			.id(1348)
    			.listName("BIOSOLIDS")
    			.label("Navigation Panel to Biosolid Lists")
    			.type("federal")
    			.visibility("PUBLIC")
		...
		listAll = factory.createProjection(ChemicalListAll.class);
		listAll.setid(1348);
		listAll.setListName("BIOSOLIDS");
		listAll.setLabel("Navigation Panel to Biosolid Lists");
		listAll.setType("federal");
		listAll.setVisibility("PUBLIC");
		listAll.setShortDescription("Biosolids lists change over time and are versioned iteratively. This panel navigates between the various versions.");
		listAll.setLongDescription("Biosolids lists change over time and are versioned iteratively. This panel navigates between the various versions which will be released over time.\r\nThe list of substances displayed below represents the latest iteration of biosolids (BIOSOLOIDS2021 - November 2021). For the versioned lists please use the hyperlinked lists below.<br/><br/>\r\n\r\n<a href='https://comptox.epa.gov/dashboard/chemical_lists/BIOSOLIDS2021' target='_blank'>BIOSOLIDS2021 - November 2021</a><br/><br/>\r\n\r\n<a href='https://comptox.epa.gov/dashboard/chemical_lists/BIOSOLIDS2022' target='_blank'>BIOSOLIDS2022 - December 2022</a><br/><br/>");
		listAll.setChemicalCount(726L);
		listAll.setCreatedAt(new Date().toInstant());
		listAll.setUpdatedAt(new Date().toInstant());
		...
		listWithDtxsids.setLabel("LIST: ACS Reagent Chemicals");
		listWithDtxsids.setType("other");
		listWithDtxsids.setVisibility("PUBLIC");
		...
		listWithDtxsids.setCreatedAt(new Date().toInstant());
    ...
    void testGetAllChemicalListDetails() throws Exception{
        final List<ChemicalListAll> lists = Collections.singletonList(listAll);
		
        when(listRepository.findByVisibilityAndIsVisibleOrderByTypeAscListNameAsc("PUBLIC", true, ChemicalListAll.class)).thenReturn(lists);
		
		mockMvc.perform(get("/chemical/list/all"))
    				.andDo(MockMvcResultHandlers.print())
    				.andExpect(status().isOk())
    				.andExpect(jsonPath("$[0].listName").value(listAll.getListName()));
    }
    ...
    void testGetAllChemicalListNames() throws Exception{
        ...
        when(listRepository.findByVisibilityAndIsVisibleOrderByTypeAscListNameAsc("PUBLIC", true, ChemicalListName.class)).thenReturn(lists);
    ...
    void testGetAllChemicalListsWithDtxsids() throws Exception{
        ...
        when(listRepository.getListsWithDtxsids("PUBLIC")).thenReturn(lists);
    ...
    void testGetChemicalListsByType() throws Exception {
		final List<Object> lists = Collections.singletonList(listAll);
		
		when(listRepository.findByTypeAndVisibilityAndIsVisibleOrderByListNameAsc(eq("federal"), eq("PUBLIC"), eq(true), any())).thenReturn(lists);
		
		mockMvc.perform(get("/chemical/list/search/by-type/{type}", "federal"))
				.andDo(MockMvcResultHandlers.print())
				.andExpect(status().isOk())
				.andExpect(jsonPath("$[0].type").value(listAll.getType()));
    ...
    void testGetChemicalListsByTypeProjection() throws Exception {
		...
		when(listRepository.findByTypeAndVisibilityAndIsVisibleOrderByListNameAsc("federal", "PUBLIC", true, ChemicalListAll.class)).thenReturn(lists);
    ...
    void testGetChemicalListNamesByType() throws Exception {
		...
		when(listRepository.findByTypeAndVisibilityAndIsVisibleOrderByListNameAsc("federal", "PUBLIC", true, ChemicalListName.class)).thenReturn(lists);
    ...
    void testGetChemicalListsWithDtxsidsByType() throws Exception {
		...
		when(listRepository.getListsWithDtxsidsByType("other", "PUBLIC")).thenReturn(lists);
    ...
    void testGetChemicalListsByName() throws Exception {
		final Optional<Object> lists = Optional.of(listAll);
		
		when(listRepository.findByListNameIgnoreCaseAndVisibilityAndIsVisible(eq("BIOSOLIDS"), eq("PUBLIC"), eq(true), any())).thenReturn(lists);
		
		mockMvc.perform(get("/chemical/list/search/by-name/{name}", "BIOSOLIDS"))
				.andDo(MockMvcResultHandlers.print())
				.andExpect(status().isOk())
				.andExpect(content().string(containsString(listAll.getListName())));
				
    }
   
    @Test
    void testGetChemicalListsByNameProjection() throws Exception {
		final Optional<ChemicalListAll> lists = Optional.of(listAll);
		
		when(listRepository.findByListNameIgnoreCaseAndVisibilityAndIsVisible("BIOSOLIDS", "PUBLIC", true, ChemicalListAll.class)).thenReturn(lists);
    ...
    void testGetChemicalListsWithDtxsidsByName() throws Exception {
		...
		when(listRepository.getListWithDtxsidsByListName("ACSREAG", "PUBLIC")).thenReturn(lists);
    ...
    void testGetChemicalListsByDtxsid() throws Exception {
		...
		when(listRepository.getListsByDtxsid("DTXSID7020182", "PUBLIC")).thenReturn(lists);
    ...
    void testGetChemicalListNamesByDtxsid() throws Exception {
		...
		when(listRepository.findByListNameInIgnoreCaseAndVisibilityAndIsVisibleOrderByListNameAsc(chemicalLists, "PUBLIC", true, ChemicalListName.class)).thenReturn(lists);
    ...
    void testGetChemicalListsCcd() throws Exception {
		...
		when(listRepository.getListsByDtxsidCcd("DTXSID7020182", "PUBLIC")).thenReturn(lists);
